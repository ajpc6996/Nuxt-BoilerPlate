  ...
  <div class="mx-auto w-full max-w-6xl flex-1 px-6 py-10 lg:px-8">
    <div>
      ...
      <p class="mt-2 max-w-2xl text-[var(--mute)]">
        View dashboards you can access in
    ...
    >
      ...
      >
        {{ items.length ? 'No dashboards match this filter.' : 'No dashboards available yet.' }}
      </p>
    </div>
  </div>
</template>
...
async function load() {
  ...
  try {
    ...
    error.value = err?.data?.statusMessage || err?.message || 'Failed to load dashboards'
    items.value = []
  }
  finally {
    pending.value = false
