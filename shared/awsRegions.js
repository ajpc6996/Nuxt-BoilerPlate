/**
 * AWS commercial region codes for connector forms.
 * Values are what the SDK expects; labels are for the dropdown.
 */
 ...
 * @param {string} key
 */
export function isAwsRegionFieldKey(key) {
 ...
 * @param {string} key
 */
export function isUrlFieldKey(key) {
  const k = String(key || '').toLowerCase()
  return /endpoint|url|uri|hostname|^host$/.test(k)
}
